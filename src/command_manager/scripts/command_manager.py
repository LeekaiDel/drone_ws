...
class CommandManager:
    ...
    def __init__(self):
        # params:
        self.delta_r = 0.2                                  # Дельта окрестность достижения точки
        ...
        self.control_cmd = CmdManagerControlCmd.FINISH      # Команда управления менеджером
        ...
        rospy.Subscriber("/task_manager/command", TaskCmd, self.task_cmd_cb, queue_size=10)
        rospy.Subscriber("/task_manager/control_cmd", CmdManagerControlCmd, self.control_cmd_cb, queue_size=10)
        ...
        while not rospy.is_shutdown():
            # print(self.control_cmd == CmdManagerControlCmd.BREAK)
            print(bool(self.command_stack))
            if self.command_stack:
                self.control_cmd = CmdManagerControlCmd.START
                for command in self.command_stack:
                    self.current_cmd = command
                    # Взлет
                    if self.current_cmd.type == TaskCmd.TAKEOFF:
                        print("command takeoff:")
                        if self.current_cmd.coordinates:
                            if self.takeoff_cmd(coordinate = self.current_cmd.coordinates[0]):
                                print("takeoff comleted")
                            else:
                                print("takeoff break")
                        else:
                            print("takeoff ERROR")
                    ...
                    # Полет в точку
                    elif self.current_cmd.type == TaskCmd.GO_TO_POINT:
                        print("command go_to_point:")
                        if self.current_cmd.coordinates:
                            if self.go_to_point_cmd(self.current_cmd.coordinates[0]):
                    ...
                    # Полет по траектории
                    elif self.current_cmd.type == TaskCmd.GO_ALONG_TRAJECTORY:
                        print("command go_along_traj_cmd:")
                        if self.go_along_trajectory_cmd(coordinates=self.current_cmd.coordinates):
                            print("command completed")
                        else:
                            print("command break")

                    # Стоп
                    elif self.current_cmd.type == TaskCmd.STOP:
                        print("command stop:") 
                        if self.stop_cmd():
                            print("command completed")
                        else:
                    ...
                    # Возврат
                    elif self.current_cmd.type == TaskCmd.RETURN:
                        pass

                    # Посадка
                    elif self.current_cmd.type == TaskCmd.LAND:
                        print("command land:") 
                        if self.land_cmd():
                            print("command completed")
                        else:
    ...
    def takeoff_cmd(self, coordinate: PoseStamped) -> bool:
        ...
        while self.reaching_delta_r(self.current_drone_pose, goal, self.delta_r):
            if self.control_cmd == CmdManagerControlCmd.BREAK:
                ...
                self.stop_cmd()
                while self.control_cmd == CmdManagerControlCmd.PAUSE:
                    pass
                else:
                    self.goal_pub.publish(goal)     
        else:
    ...
    def go_along_trajectory_cmd(self, coordinates: list) -> bool:
        if coordinates:
            for pose_point in coordinates:
                ...
                while self.reaching_delta_r(self.current_drone_pose, goal, self.delta_r):  # and self.r_vel >= 0.03
                    if self.control_cmd == CmdManagerControlCmd.BREAK:
                        self.stop_cmd()
                        return False    # Если команда прервана возвращаем false
    ...
    def return_cmd(self, return_path: list) -> bool:
        ...
        for goal_point in return_path:
            if self.control_cmd == CmdManagerControlCmd.BREAK:
                ...
                while self.reaching_delta_r(self.current_drone_pose, goal_point, self.delta_r) or not self.control_cmd == CmdManagerControlCmd.BREAK or not self.control_cmd == CmdManagerControlCmd.PAUSE:
                    pass
        return True
